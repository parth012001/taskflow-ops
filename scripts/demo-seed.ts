 ...
 */
import { PrismaClient, TaskStatus, TaskPriority, TaskSize, AssignedByType } from "@prisma/client";
...
async function main() {
  ...
  const manager = await prisma.user.findUniqueOrThrow({
    where: { email: "manager1@taskflow.com" },
  });
  const employee1 = await prisma.user.findUniqueOrThrow({
    where: { email: "employee1@taskflow.com" },
  });
  const employee2 = await prisma.user.findUniqueOrThrow({
    where: { email: "employee2@taskflow.com" },
  });
  ...
  await createTask({
    ...
    description:
      "Review and update payment terms across all active Q2 vendor contracts to reflect new 45-day net policy.",
  ...
  await createTask({
    ...
    description:
      "Cross-check all vendor invoices received in March against purchase orders and delivery receipts.",
    ownerId: employee1.id,
    status: TaskStatus.IN_PROGRESS,
  ...
  await createTask({
    ...
    description:
      "Complete vendor onboarding checklist: NDA, tax forms, bank details, quality certification.",
    ownerId: employee1.id,
  ...
  await createTask({
    ...
    description:
      "Audit all safety gear procurement records and vendor certifications for Site B warehouse.",
    ownerId: employee1.id,
    status: TaskStatus.ACCEPTED,
  ...
  await createTask({
    ...
    description:
      "Completed negotiation for 15% volume discount on quarterly steel orders exceeding 50 tonnes.",
    ownerId: employee1.id,
  ...
  await createTask({
    ...
    description:
      "Compiled performance scorecards for all 12 active vendors based on delivery accuracy and quality metrics.",
    ownerId: employee1.id,
  ...
  await createTask({
    ...
    kpiBucketId: kpi(0),
    assignedByType: "MANAGER",
    deadlineDaysFromNow: -1, // overdue
  });
  await createTask({
    title: "Update supplier contact directory",
    description:
      "Refresh contact details for all 45 active suppliers in the vendor management system.",
    ownerId: employee2.id,
    status: TaskStatus.COMPLETED_PENDING_REVIEW,
    priority: "NOT_URGENT_NOT_IMPORTANT",
    size: "EASY",
  ...
  await createTask({
    ...
    description:
      "Cost analysis comparing DHL Express, BlueDart, and Delhivery for standard parcel shipments.",
    ownerId: employee2.id,
    status: TaskStatus.IN_PROGRESS,
    priority: "NOT_URGENT_IMPORTANT",
    size: "MEDIUM",
  ...
  await createTask({
    ...
    description:
      "Prepare service level agreement with updated delivery timelines and penalty clauses.",
    ownerId: employee2.id,
    status: TaskStatus.ACCEPTED,
    priority: "URGENT_NOT_IMPORTANT",
    size: "MEDIUM",
    ...
    data: [
      {
        ...
        content:
          "The procurement freeze for non-essential items has been lifted effective immediately. Please resume pending purchase orders and submit new requests through the standard approval workflow.",
        type: "POLICY",
        priority: "HIGH",
      ...
      },
      {
        ...
        content:
          "Please join us in welcoming Swati Kulkarni who joins our Procurement team as a Purchase Coordinator. Swati brings 3 years of supply chain experience from her previous role.",
        type: "GENERAL",
        priority: "NORMAL",
        authorId: admin.id,
        isActive: true,
      ...
      },
      {
        ...
        content:
          "All department heads must submit their quarterly vendor evaluation reports by March 28th. Templates are available in the shared drive under /Reports/Q1-2026/.",
        type: "EVENT",
        priority: "NORMAL",
        authorId: manager.id,
        isActive: true,
